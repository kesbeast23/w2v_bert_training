...
class DataTrainingArguments:
    ...
    streaming: bool = field(
        default=False,
        metadata={"help": "Whether to use streaming mode for the dataset."},
    )
    eval_dataset_name: str = field(
        default=None,
        metadata={"help": "Path or name of the evaluation dataset (if different from training dataset)."}
    )
    eval_dataset_config_name: str = field(
        default=None,
        metadata={
            "help": "The configuration name of the evaluation dataset to use (if different from training dataset)."
        },
    )
...
def main():
    ...
    # Explicitly load token from environment if not provided
    if data_args.token is None:
        data_args.token = os.getenv("HF_TOKEN")
    ...
    if training_args.do_eval:
        eval_dataset_name = data_args.eval_dataset_name if data_args.eval_dataset_name is not None else data_args.dataset_name
        eval_dataset_config_name = data_args.eval_dataset_config_name if data_args.eval_dataset_config_name is not None else data_args.dataset_config_name
